...
import mertics_np as mertics
import model_np as model
from dataloader import minst
# import psmo as model
from kernels_np import LinearKernel, RBFKernel
...
def one_vs_one():
    ...
    for i in range(10):
        for j in range(10):
            data = train_data[np.where((train_label == i) | (train_label == j))]
            label = train_label[np.where((train_label == i) | (train_label == j))]
            ...
            for j in range(i):
                ...
                for k in range(len(train_data)):
                    if prediction[k] == 1:
                        ...
                        train_count[k][j] += 1
        train_result = np.argmax(train_count, axis=1)
    ...
    # test
    test_count = np.zeros((len(test_data), 10), dtype=np.int64)
    test_res = np.zeros((10, 10, len(test_data)), dtype=np.int64)
    for i in trange(10):
        for j in range(i):
            if i == j:
                continue
            ...
            for k in range(len(test_data)):
                if prediction[k] == 1:
                    ...
                    test_count[k][j] += 1
    test_result = np.argmax(test_count, axis=1)
...
if __name__ == '__main__':
    ...
    rbf_kernel = RBFKernel(np.sqrt(1 / (2 * gamma)))
