...
class SMO:
    def __init__(self, x, y, c: float, tol: float, max_passes: int, kernel: KernelBase):
        """
        构造函数
        :param x: 特征向量，二维数组
        :param y: 数据标签，一维数组
        ...
        self.alpha = jnp.zeros(self.n)  # 初始化alpha
        self.b = 0.0
        self.error_cache = jnp.zeros((self.n, 2))  # 错误缓存

        self.SV = ()  # 最后保留的支持向量
        self.SVIndex = None  # 支持向量的索引

        self.K = jnp.zeros((self.n, self.n))  # 先求内积，
    ...
    def calc_E(self, k):
        ...
        return jnp.dot(self.alpha * self.y, self.K[:, k]) + self.b - self.y[k]

    def update_Ek(self, k):
        # self.error_cache[k] = [1, (self.calc_E(k))]
        self.error_cache = self.error_cache.at[k].set([1, (self.calc_E(k))])

    def select_j_rand(self, i, m):
        j = i
        while j == i:
            j = jax.random.randint(key, [0], 0, m)
        return j
    ...
    def select_j(self, i, Ei):
        ...
        j = 0
        E_j = 0.0
        valid_e_cache_list = jnp.nonzero(self.error_cache[:, 0])[0]
    ...
    def update_alpha(self, i: int) -> Literal[0, 1]:
        ...
        if (self.y[i] * E_i < -self.tol and self.alpha[i] < self.c) or \
            ...
            if self.y[i] != self.y[j]:
                L = jnp.maximum(0, self.alpha[j] - self.alpha[i])
                H = jnp.minimum(self.c, self.c + self.alpha[j] - self.alpha[i])
            else:
                L = jnp.maximum(0, self.alpha[j] + self.alpha[i] - self.c)
                H = jnp.minimum(self.c, self.alpha[i] + self.alpha[j])
            if L == H:
                return 0
            ...
            self.alpha=self.alpha.at[j].set(self.y[j] * (E_i - E_j) / eta)

            # self.alpha[j] = self.clip_alpha(self.alpha[j], L, H)

            self.alpha=self.alpha.at[j].set(self.clip_alpha(self.alpha[j], L, H))

            self.update_Ek(j)

            if jnp.abs(alpha_j_old - self.alpha[j]) < self.eps:
                # 目标迭代完成，不需要再迭代，而且所有参数已经保留，理论上是不应该保留当前的参数的，但是也正因为反正相差不多，所以可以
                return 0

            # self.alpha[i] += self.y[i] * self.y[j] * (alpha_j_old - self.alpha[j])
            self.alpha = self.alpha.at[i].set(self.y[i] * self.y[j] * (alpha_j_old - self.alpha[j]))
            self.update_Ek(i)
    ...
    def fit(self):
        ...
        for i in range(self.n):
            for j in range(self.n):
                self.K = self.K.at[i, j].set(self.kernel(self.x[i], self.x[j]))
        ...
        while passes < self.max_passes and ((num_changed_alpha_pair > 0) or entry_set):
            ...
            if entry_set:
                ...
                non_bounds = jnp.nonzero((self.alpha > 0) * (self.alpha < self.c))[0]
                for i in non_bounds:
                    num_changed_alpha_pair += self.update_alpha(i)
                passes += 1
            if entry_set:
                ...
                entry_set = True
        # 保存模型参数
        self.SVIndex = jnp.nonzero(self.alpha)[0]
    ...
    def calcw(self):
        ...
        """
        for i in range(self.n):
            self.w += jnp.dot(self.alpha[i] * self.y[i], self.x[i, :])
    ...
    def predict(self, input_x):
        ...
        result_np = jnp.where(forward_value > 0, 1, -1)
        return result_np
    ...
    def forward(self, input_x):
        ...
        :return: 一个列表，包含结果
        """
        forward_value = jnp.sum(self.SVAlpha * self.SVLabel * self.kernel(self.SV, np.expand_dims(input_x, axis=1)),
                                axis=-1) + self.b
